...
export function RenameModal({
  ...
  useEffect(() => {
    if (open) form.setFieldsValue({ name: currentName });
  ...
  const genericError = error instanceof Error && !(error instanceof ApiErrorThrown) ? error.message : null;
  ...
  return (
    ...
    >
      <Form form={form} layout="vertical" preserve={false}>
        ...
        >
          <Input autoFocus disabled={isPending} placeholder={`New ${resourceLabel} name`} />
        </Form.Item>
        {nameTaken ? (
          <Alert
            type="error"
