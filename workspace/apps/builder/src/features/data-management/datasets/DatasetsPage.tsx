...
export function DatasetsPage() {
  ...
  const visibleRows =
    normalisedQuery.length === 0
      ? allRows
      : allRows.filter((r) =>
          r.name.toLowerCase().includes(normalisedQuery),
        );
  ...
  if (datasets.isLoading || workspaces.isLoading) {
    ...
    body = (
      <div
        data-component="DatasetsNoMatch"
      ...
      >
        ...
        <Typography.Text type="secondary">
          ...
          <Typography.Link onClick={() => setQuery("")}>
            clear the search
          </Typography.Link>
          .
        </Typography.Text>
...
function DatasetTable({ rows, workspaceById, onWorkspaceClick }: TableProps) {
