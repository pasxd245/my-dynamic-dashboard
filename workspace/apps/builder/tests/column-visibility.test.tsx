...
describe("DatasetDetailPage — Profiling section (R154)", () => {
  function openDrawer() {
    return waitFor(() =>
      expect(document.querySelector('[data-component="ColumnsToolbarButton"]')).toBeInTheDocument(),
    ).then(() => {
  ...
  it("opening the drawer fetches the profile and renders a stat row per column (+ format cell)", async () => {
    // Default MSW profile handler → contract-validated body (proves the mock conforms).
    server.use(http.get("*/datasets/:id", () => HttpResponse.json(datasetWithHidden())));
    renderDetail();
    await openDrawer();

    // The Profiling section loads its rows (one per column, incl. hidden).
    await waitFor(() =>
      expect(document.querySelectorAll('[data-component="PropertiesDrawerProfileRow"]').length).toBe(
        MOCK_DATASET.columns.length,
      ),
    );
    // A stat line is rendered (en: "N distinct").
    const profile = document.querySelector('[data-component="PropertiesDrawerProfile"]') as HTMLElement;
    expect(profile.textContent).toContain("distinct");
    // `format` rides the profile → the date/datetime columns show a format cell.
    const formatCells = document.querySelectorAll('[data-component="PropertiesDrawerFormat"]');
    expect(formatCells.length).toBeGreaterThan(0);
    expect(document.querySelector('[data-component="PropertiesDrawerFormat"][data-column="won_at"]')).toBeInTheDocument();
  });

  it("is LAZY — the profile is not fetched until the drawer opens", async () => {
    let profileCalls = 0;
    server.use(
      http.get("*/datasets/:id", () => HttpResponse.json(datasetWithHidden())),
      http.get("*/datasets/:id/profile", () => {
        profileCalls += 1;
    ...
    renderDetail();
    // Wait for the page (rows table) — the profile must still not have fired.
    await waitFor(() =>
      expect(document.querySelector('[data-component="PagedRowsTable"]')).toBeInTheDocument(),
    );
  ...
  it("shows an error with a retry that refetches on failure", async () => {
    server.use(
      http.get("*/datasets/:id", () => HttpResponse.json(datasetWithHidden())),
      http.get("*/datasets/:id/profile", () => HttpResponse.json({ code: "internal_error" }, { status: 500 })),
    );
    renderDetail();
    await openDrawer();

    await waitFor(() =>
      expect(document.querySelector('[data-component="PropertiesDrawerProfileError"]')).toBeInTheDocument(),
    );

    // Recover: the retry refetches and the rows render.
    server.use(
      http.get("*/datasets/:id/profile", () =>
        HttpResponse.json({
          datasetId: DS_ID,
          ...
          sampledRows: null,
          columns: [
            { name: "deal_id", dtype: "string", format: null, nullCount: 0, nullPct: 0, distinctCount: 1, min: null, max: null, sample: ["X"] },
          ],
        }),
      ),
    );
    fireEvent.click(document.querySelector('[data-component="PropertiesDrawerProfileRetry"]') as HTMLElement);
    await waitFor(() =>
      expect(document.querySelector('[data-component="PropertiesDrawerProfileRow"]')).toBeInTheDocument(),
    );
  });
});
