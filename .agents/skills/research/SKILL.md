...
when_to_use: User asks to research, investigate, compare, evaluate, survey, find prior art, assess current state, check known practice, or prepare a sourced decision brief. Also invoke from any primary skill or task that needs extra information beyond the round file (e.g. flow-selector condition #2 prior-art check).
allowed-tools: Read, Grep, Glob, WebFetch, WebSearch, Bash(python3 *), Bash(pip *), Bash(crawl4ai-* *)
metadata:
  author: a2scaffold
  version: '1.0'
---
