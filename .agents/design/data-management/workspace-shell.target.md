...
| **Workspaces feature** — `/data-management/workspaces` sub-route, card grid, Workspace data model, TanStack Query first use, `GET /workspaces` backend stub           | (entry N: DEFER → ADOPT-VIA-ROUND in R13) | R13   | Designed in R11 ([workspaces.md](workspaces.md)).                                                                        |
