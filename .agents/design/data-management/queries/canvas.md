...
- **Promote** pushes a query-owned rel up into the governed ER via the existing
  ...
  - **R167 removed an EARLIER guard, for a different shape.** The old `promotable` flag existed
    ...
    conclusion drawn at the time, _"there is no longer a shape this could be offered for and then
    rejected"_, held only for it. The copied-and-in-sync shape was always the other one.
    Un-offerability is now back to being enforced by a check, because the model does not encode
    it: D4's rule (_unofferable at the gesture, never an error at run_) is about what the user
    meets, not about which layer supplies the guarantee.
