# Multi-join — chain a Query across more than two related datasets

**Concept**: a **multi-join** is the fourth construction mode of a
[Query](saved-query.md): a Query whose definition chains **two or more** governed
...
**single** edge, R71) into an **ordered chain of edges**, executed by growing
`query_joined_rows` from a fixed two-source join into a **fold over the chain**.
The Query keeps its `qr_` identity, its catalog, its
`/data-management/queries/:id` detail, the [builder](query-construction.md) R72
shipped, and its run / preview routes. R73 ships the **linear** chain (each hop
extends from the previous dataset) editable in R72's builder; the **free-form
...
- **The linear-chain invariant (J-4).** The chain is a **path**, not a tree:
  `joins[0]`'s left dataset is the Query's **source** (`datasetId`), and each
  subsequent `joins[k]`'s left dataset is the **tail** — the right dataset of
  `joins[k-1]`. So each hop introduces **exactly one new dataset**, and the chain's
  driving source never changes (the [left-source-stable](query-construction.md)
  rule R72 set, generalized). A hop onto a dataset **already in the chain** (a
  star/tree) is **R74's** canvas.
...
run path — routers/queries.py (extended; sketch)
  ...
  3. else (chain present):
     a. resolve the chain in order: D0 = query.datasetId; for each hop k, load
        joins[k].rel (404 if deleted), require rel.left == tail, set tail = rel.right
        → a non-linear / non-tail edge is a 422 (linear-chain invariant)
     b. recompute each edge's status vs CURRENT schemas (reuse _compute_status)
     ...
     d. re-validate every FilterAtom against the EFFECTIVE columns
        (reuse build_definition_predicates) → a drifted atom: 409 query_stale
     e. rows, total = query_joined_rows(sources=[D0..Dn], ons=[(k0),(k1),…],
           type='inner', columns=effective, page=…, page_size=…, q, filters, advanced)
  4. return the SAME RowsPage shape (+ resolvedColumns when chained)
```

`query_joined_rows` grows from a fixed two-source join into a **fold**: it opens
the same ephemeral `duckdb.connect(":memory:")` and builds
`FROM read_parquet(?) AS T0 INNER JOIN read_parquet(?) AS T1 ON T0."k0"=T1."k0'"
INNER JOIN read_parquet(?) AS T2 ON T1."k1"=T2."k1'" …`, composing the **reused**
fragment builders with **source-qualified** identifiers derived from the combined
column index. No new Parquet, no result cache. _Trigger to materialize:_ a chain
too slow / too row-multiplying to be interactive at real data scale (Scope).
...
- **Add a hop** — `[+ Add a join]` lists edges whose left dataset is the **tail**;
  choosing one appends a `JoinStep` and extends the effective column space. When the
  tail has **zero valid** outgoing edges (none declared, or all stale), `[+ Add a
join]` is **disabled** with the same guiding tooltip R71 set — _"Declare a
  relationship for this dataset first"_, linking to the workspace
  [Relationships](../workspaces/relationships.md) view — so the chain is never
  offered with nothing to extend onto (no dead-end empty `<Select>`).
- **Remove a hop** — `[Remove]` drops the **last** hop, re-binding predicate
  columns to the shorter space; predicates over a now-absent column flag invalid
  **in the builder** (caught before save), mirroring `query_stale`.
...
- **`_shared/query.yaml`** is where `JoinStep` lives (R71); R73 changes the field
  from `join` to `joins` there, so all routes that `$ref` it inherit the chain.
- **No new error codes** — `relationship_stale` / `query_stale` / `not_found` /
  the `422` envelope all exist (R69/R70/R71); the chain **consumes** them per hop.
- **Open contract questions (flag for the Contract gate, not pre-decided):**
  (1) the **legacy `join` migration seam** — a read-shim at the boundary vs. a
  one-time rewrite of stored `definition_json` (J-3 fixes the shape; the seam is a
  Backend-gate call). (2) Whether the per-hop stale `409` should name the **hop
  index** in the error body (UX-dependent — confirmed at F1).
...
- Generalizing `QueryDefinition.join` (singular) → **`joins: JoinStep[]`** (an
  ordered **linear** chain) with the back-compat read shim; the generalized
  effective column space + collision rule; source-qualified predicate resolution
  across N sources.
- Growing `query_joined_rows` from a two-source join into a **multi-hop inner-join
  fold** (reusing the predicate fragment builders); the **per-hop** `409
relationship_stale` gate.
- The **chain editor** in R72's builder (append a hop from the tail / remove the
  last hop) + the read-only chain summary; the chained result/preview via the
  reused `<PagedRowsView>`.

### OUT of scope (deferred with named triggers)

- **The free-form visual builder canvas / source graph + non-linear topology**
  (a dataset joined to 2+ others — a star/tree) → **R74** (J-1′). _Trigger: a Query
  must join one dataset to two or more others — a branch the linear path cannot
  express._
- **Left / right / outer joins; composite / multi-column keys; self-joins;
  cross-workspace joins** → future (R70/R71 triggers hold); R73 chains
  **single-column, within-workspace, inner** hops.
...
- **Row-explosion guard / aggregation / dedup** on multiplying chains → future.
  _Trigger: a real report's chain multiplies rows past usability (likelier on a
  chain than a single join)._
- **Removing a non-last hop / re-ordering hops** → R74 (the canvas's editing
  model); R73 edits only the chain's **tail**.
- **Renaming a Query from the builder; result materialization / pinned snapshots;
  Excel export of a chain; dashboards** → downstream value-out; preview + save stay
  live re-run.
