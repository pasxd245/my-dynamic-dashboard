...
| **within-group column** | appends a column whose value is an aggregate over the **group of other rows** it belongs to; the row count is **unchanged** | yes (`group_column`) — the rest of the family (% of total · running total · rank within group · vs prior period) needs an in-window `ORDER BY` + frame and is program item 2 |
