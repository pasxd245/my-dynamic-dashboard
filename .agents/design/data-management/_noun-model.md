...
> **Status 2026-08-13 (R166 D gate): all three are withdrawn; D5 stays OPEN.** The surfaces go at
> **R166**, the engine at **R167** — item 3 was split by layer once a code walk found the engine
> half is neither a deletion nor net-negative (see the program plan § Item 3's layer split).
> **Withdrawing an affordance is not retiring a capability**: for one round the engine still
> accepts `qr_` operands over the API that no UI offers, which is today's state exactly and is
> therefore not a regression. D5 closes when the engine does.
