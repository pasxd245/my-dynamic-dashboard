...
**Replace before you remove — and it was.** Composition is **retired** (§ Named debt D5, closed
R167): the within-group family (R163) and the ordered-window family (R165) landed first, then
**Duplicate** replaced the variant need (R166), and only then did the capability come out (R167).
Reversing that order would have left the human more blocked than before the program started; the
acceptance walk that confirmed Duplicate serves the need is what licensed the removal.
...
| ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **D1** | **Steps dropped in composition** — a query used as a source loses its `aggregate/derive/filter/top_n/sort/select/date_bucket`; the join silently runs against un-shaped rows.                                                                                                                                           | `resolve_source` bakes source+joins+own-filters only ([query_engine.py:87-100](../../../workspace/apps/backend/app/query_engine.py#L87)); `run_steps` runs only at top-level ([queries.py](../../../workspace/apps/backend/app/routers/queries.py)).                                           | No composition ⇒ no step-drop.                                                                                                                          | **DOES NOT DISSOLVE — corrected 2026-08-13 (R166 D gate).** It **relocates**. `resolve_source`'s `qr_` branch is also how a **Workflow** reads its sources (`WorkflowSourceId` is `qr_`/`wf_`, never `ds_`), and `build_consolidated_relation` never calls `run_steps` either — so a Workflow consolidating a _shaped_ query reads its **un-shaped** rows and **materializes them to `output.parquet`**, which is worse than the query path because the wrong rows are frozen rather than recomputed. This row's own escape clause (_"if item 3 slips, this reverts to a bug that must be fixed on its own"_) fires for a reason it did not anticipate: **item 3 lands and D1 survives it.** **CLOSED at R168 (2026-08-14).** Ruled a **bug**, not the definition — the counter-reading ("a Workflow shapes its source's raw rows: one layer, not two") was not self-consistent as shipped, since a `wf_` source already resolved POST-steps while a `qr_` source resolved PRE-steps, and the builder already offered a query's `resolvedColumns` (post-step) to build workflow steps the run then refused. Repaired in `resolve_source`'s `qr_` branch (`_resolve_query_source`) rather than in the consolidation path, so the resolver's contract is true for every consumer. **Two shaping altitudes**: per-source shaping is the Query's and arrives applied; post-union shaping is the Workflow's. |
| **D2** | **Shared-leaf `cyclic_join`** — two queries derived from one dataset can't be joined.                                                                                                                                                                                                                                   | Tree invariant rejects any leaf-overlap ([query_engine.py:212-218](../../../workspace/apps/backend/app/query_engine.py#L212)).                                                                                                                                                                 | Not applicable — there are no query operands to overlap.                                                                                                | **Removed by decision, not fixed.** Joining two queries is out of scope; the capability is deliberately gone. A later reader must **not** read this as a bug that got solved. What replaces it is the within-group column (R162), not a relaxed invariant. **Clarified 2026-08-13**: what is deleted is the **set-overlap** test (a `qr_` right brings a _set_ of leaves). The `cyclic_join` **error code and check survive** — for a `ds_` right they are the **self-join boundary**, which stays rejected. The check degenerates from a set intersection to a single-id membership test.                                                                                                                                                 |
| **D3** | **Workflow as a separate noun** — full table + routes + 3 FE surfaces for what may be a live/frozen toggle.                                                                                                                                                                                                             | [workflows/workflows.md](workflows/workflows.md); `wf_` model, `/workflows` routes, catalog/builder/detail.                                                                                                                                                                                    | Settled at program item 4.                                                                                                                              | **CLOSED at R168 (2026-08-14).** The noun stands, and it is **not** a live/frozen toggle on a Query. It is for exactly two things a Query cannot express: **consolidate** (`UNION ALL BY NAME` over ≥1 saved query — there is no `query ∪ query`) and **materialize** (a frozen typed parquet + captured schema, readable as a stable source; a Query is live-only). Its `steps` are borrowed, not a third thing, and their justification narrowed to **post-union shaping** once D1 closed. Frozen was ruled **permanent** — which retired the `composition_cycle` error code, since a live-resolving source was its only reactivating condition. |
...
| **D5** | ~~The shipped product exceeds the concept — composition is built, twice, and actively OFFERED.~~ | — | Both operands dataset-only, every entry point withdrawn. | **CLOSED at R167 (2026-08-14).** R166 withdrew all three surface entry points (FE-only, contract-safe); R167 narrowed the wire (`sourceId` and `rightSourceId` are `^ds_…`; the polymorphic `SourceId` alias is **retired outright** — it had 4 users, all query-side) and the engine. **Three corrections to the intended remedy, all from the code**: `resolve_source`'s `qr_` branch is **not deleted** — it is Workflow's reader (finding A), now reachable from exactly ONE call site because the join path resolves a dataset leaf directly; **`cyclic_join` is not deleted** — it is the self-join boundary, and only its set-shaped form collapsed (finding B); and **`composition_cycle` is not retired** but kept **DORMANT** — it is unreachable only because a workflow source resolves frozen rather than live, which is item 4's question, so deleting it would mean re-deriving it a round later. _(R168 answered that question — frozen, permanently — and retired the error CODE while keeping the guard; see D3.)_ Zero composed queries existed (`data/app.sqlite`, re-confirmed 2026-08-14), so the migration stance is reject-at-write, not a data job. |
...
| #   | Entry point                                                                        | Mechanism                                                                               | Withdrawn at R166 by                                                                             |
| --- | ---------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| 1   | **`[Build on this query]`** on the Query detail header → `/queries/new?base=<qr_>` | composition as the **driving base**                                                     | replaced by **`[Duplicate]`** ([queries.md § Duplicate](queries/queries.md))                     |
| 2   | The canvas **"Add a source"** picker's **"Saved queries"** option group            | a `qr_` joined in on the **right of a hop** (R91/R92) — a _different_ mechanism from #1 | **removed, no replacement** — the picker is a flat dataset list ([canvas.md](queries/canvas.md)) |
| 3   | The `?base=` route itself, reachable by URL without #1                             | same as #1                                                                              | route + `QueryCreatePage` **deleted** ([query-construction.md](queries/query-construction.md))   |
...
> **Status 2026-08-14 (R167): all three are withdrawn AND the engine is narrowed — D5 is CLOSED.**
> The surfaces went at **R166**, the engine at **R167** — item 3 was split by layer once a code walk
> found the engine half is neither a deletion nor net-negative (see the program plan § Item 3's
> layer split). The section below is retained as the record of what the three entry points WERE:
> **withdrawing an affordance is not retiring a capability**, and for exactly one round the engine
> still accepted over the API what no UI offered. That gap is now shut from both sides.

**Why they stayed lit until R166.** [Replace before you
...
`cyclic_join` dead end that started R160** — so the product kept inviting the failure until R166.
The open call this section left the human — _whether to withdraw the affordances **earlier** than
the engine (a cheap FE-only change, once R163 makes the replacement real)_ — was **answered yes on
2026-08-13**, and is what cut item 3 into 3a (R166, FE) and 3b (R167, engine).
