...
- [ ] UI bring-up: workflow stage nav renders, active stage is visually
      unambiguous, status tags match design tokens
- [ ] UI bring-up: connection status banner renders for `ready` / `degraded` /
      `unavailable` states with correct semantic color
- [ ] UI bring-up: prerequisite callout (locked stage) renders as AntD `Alert`
      and the "Go to previous stage" / "Resolve context" actions still work
...
- [ ] → context/ — `STATUS_COLOR` mapping pattern (status enum → AntD `Tag`
